...
class IssueSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('id', 'title', 'description', 'status', 'status_display', 'priority', 
                  'priority_display', 'created_by', 'created_by_name', 'assigned_to', 
                  'assigned_to_name', 'created_at', 'updated_at', 'course_unit', 
                  'college', 'comments_count')
        read_only_fields = ('created_by', 'created_at', 'updated_at', 'status_display', 
                           'priority_display', 'comments_count')
    ...
    def get_status_display(self, obj):
        return dict(Issue.STATUS_CHOICES).get(obj.status, obj.status)
    
    def get_priority_display(self, obj):
        return dict(Issue.PRIORITY_CHOICES).get(obj.priority, obj.priority)
    ...
    def update(self, instance, validated_data):
        ...
        # Create notifications for priority changes
        if 'priority' in validated_data and old_priority != instance.priority:
            # Notify the assigned user
            if instance.assigned_to and instance.assigned_to != self.context['request'].user:
                Notification.objects.create(
                    user=instance.assigned_to,
                    notification_type=Notification.ISSUE_UPDATED,
                    issue=instance,
                    message=f"Priority of issue '{instance.title}' has been changed to {instance.priority}"
                )
            ...
            if instance.created_by != self.context['request'].user:
                Notification.objects.create(
                    ...
                    notification_type=Notification.ISSUE_UPDATED,
                    issue=instance,
                    message=f"Priority of your issue '{instance.title}' has been changed to {instance.priority}"
                )
...
class CommentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Comment
        fields = ('id', 'issue', 'content', 'created_by', 'created_by_name', 
                 'created_by_role', 'created_at')
        read_only_fields = ('created_by', 'created_at', 'created_by_name', 'created_by_role')
...
class NotificationSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Notification
        fields = ('id', 'user', 'notification_type', 'issue', 'issue_title', 
